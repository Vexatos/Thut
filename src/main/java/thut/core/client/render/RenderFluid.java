    ...
    if(!renderTop && !renderBottom && !renderSides[0] && !renderSides[1] && !renderSides[2] && !renderSides[3]) {
      ...
      }
      {
        if(renderer.renderAllFaces || renderTop) {
          ...
          IIcon IIconStill = block.getIcon(world, x, y, z, 1);

          float flowDir = (float) BlockFluidBase.getFlowDirection(world, x, y, z);

          if(flowDir > -999.0F) {
            IIconStill = block.getIcon(world, x, y, z, 1);
          }
          ...
          if(flowDir < -999.0F) {
            u2 = IIconStill.getInterpolatedU(0.0D);
            v2 = IIconStill.getInterpolatedV(0.0D);
            u1 = u2;
            v1 = IIconStill.getInterpolatedV(16.0D);
            u4 = IIconStill.getInterpolatedU(16.0D);
            v4 = v1;
            u3 = u4;
            v3 = v2;
          } else {
            ...
            u2 = IIconStill.getInterpolatedU(8.0F + (-zFlow - xFlow) * 16.0F);
            v2 = IIconStill.getInterpolatedV(8.0F + (-zFlow + xFlow) * 16.0F);
            u1 = IIconStill.getInterpolatedU(8.0F + (-zFlow + xFlow) * 16.0F);
            v1 = IIconStill.getInterpolatedV(8.0F + (zFlow + xFlow) * 16.0F);
            u4 = IIconStill.getInterpolatedU(8.0F + (zFlow + xFlow) * 16.0F);
            v4 = IIconStill.getInterpolatedV(8.0F + (zFlow - xFlow) * 16.0F);
            u3 = IIconStill.getInterpolatedU(8.0F + (zFlow - xFlow) * 16.0F);
            v3 = IIconStill.getInterpolatedV(8.0F + (-zFlow - xFlow) * 16.0F);
          }
        ...
        for(int side = 0; side < 4; ++side) {
          ...
          IIcon IIconFlow = block.getIcon(world, x, y, z, side + 2);
          ...
          if(renderer.renderAllFaces || renderSides[side]) {
            ...
            float u1Flow = IIconFlow.getInterpolatedU(0.0D);
            float u2Flow = theFluid.solid ? IIconFlow.getMaxU() : IIconFlow.getInterpolatedU(8.0D);
            float v1Flow = IIconFlow.getInterpolatedV((1.0D - ty1) * 16.0D * 0.5D);
            float v2Flow = IIconFlow.getInterpolatedV((1.0D - ty2) * 16.0D * 0.5D);
            float v3Flow = theFluid.solid ? IIconFlow.getMaxV() : IIconFlow.getInterpolatedV(8.0D);
  ...
  public boolean renderFluidOverLay(IBlockAccess world, int x, int y, int z, Block block, RenderBlocks renderer) {
    ...
    if((up == Blocks.water || up == Blocks.flowing_water)) {
      ...
      for(int side = 0; side < 4; ++side) {
        ...
        IIcon IIconFlow = block.getIcon(side + 2, bMeta);
        ...
        if(renderer.renderAllFaces || renderSides[side]) {
          ...
          float u1Flow = IIconFlow.getInterpolatedU(0.0D);
          float u2Flow = IIconFlow.getInterpolatedU(8.0D);
          float v1Flow = IIconFlow.getInterpolatedV((1.0D - ty1) * 16.0D * 0.5D);
          float v2Flow = IIconFlow.getInterpolatedV((1.0D - ty2) * 16.0D * 0.5D);
          float v3Flow = IIconFlow.getInterpolatedV(8.0D);
          tessellator.setBrightness(block.getMixedBrightnessForBlock(world, x2, y, z2));
          float sideLighting = 1.0F;
    ...
    if(!renderTop && !renderBottom && !renderSides[0] && !renderSides[1] && !renderSides[2] && !renderSides[3]) {
      ...
      }
      {
        if(renderer.renderAllFaces || renderTop) {
          ...
          IIcon IIconStill = block.getIcon(1, bMeta);

          float flowDir = (float) BlockFluidBase.getFlowDirection(world, x, y, z);

          //                if (flowDir > -999.0F) {
          //                    IIconStill = block.getIcon(2, bMeta);
          //                }
