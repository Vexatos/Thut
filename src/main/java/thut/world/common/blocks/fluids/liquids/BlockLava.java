...
public class BlockLava extends BlockFluid //implements IHeatSource
{
  ...
  public void tickSides(World worldObj, int x, int y, int z, int rate) {
    ...
    for(int[] side : sides) {
      Vector3 vec = new Vector3(x + side[0], y + side[1], z + side[2]);
      Block blocki = vec.getBlock(worldObj);
      if(blocki instanceof BlockFluid && ((BlockFluid) blocki).solidifiable || blocki == water) {
        worldObj.scheduleBlockUpdate(x + side[0], y + side[1], z + side[2], blocki, rate);
      }
    }
  }
          ...
          for(int k1 = y - 1; k1 <= y + 4; ++k1) {
            if(i1 != x || k1 != y || j1 != z) {
              ...
              if(i2 > 0) {
                ...
                if(j2 > 0 && par5Random.nextInt(l1) <= j2 && (!worldObj.isRaining() || !worldObj.canLightningStrikeAt(i1, k1, j1)) && !worldObj.canLightningStrikeAt(i1 - 1, k1, z) && !worldObj
                    .canLightningStrikeAt(i1 + 1, k1, j1) && !worldObj.canLightningStrikeAt(i1, k1, j1 - 1) && !worldObj.canLightningStrikeAt(i1, k1, j1 + 1)) {
