...
public class BlockDust extends BlockFluid {
  ...
  public void setData() {
    //		if(data==null)TODO block combnations for dust
    //		{
    ...
    //		combinationList.add(4096*Blocks.dust.blockID);
    //
    //		for(int i = 0;i<4;i++){
  ...
  public void updateTick(World worldObj, int x, int y, int z, Random r) {
    ...
    if(worldObj.getBlock(x, y, z) == this) {
      ...
      if(Math.random() > 0.999 && meta == 15) {
        if(idUp instanceof BlockFluid && metaUp == 0) {
          worldObj.setBlock(x, y, z, ThutBlocks.worldGen, 1, 2);
        }
      }
      if(isFloating(worldObj, x, y, z)) {
        here.setAir(worldObj);
      }
      if(worldObj.getBlock(x, y, z) == this && !ConfigHandler.deleteAsh) {
  ...
  public String getItemIconName() {
    return WorldCore.TEXTURE_PATH + "concreteDust";
  }
  ////////////////////////////////////////////Plant stuff////////////////////////////////////////////////////////////////
